...
Uso da IA   : O meu código ficou dando 5% de erro no beecrownd e usei a IA para apontar as linhas
              visto que tinha 90 linhas para verificar. Foram pequenos erros de parametros e não 
              sabia que quando aloco a matriz dinamicamente preciso passar como parametro dinamicamente
...
//Recebe a matriz M e a quantidade de instâncias. Retorna 1 se ok e 0 se falso
int verificar(int **M, int inst);

//Recebe o vetor e seu tamanho e vê se há elementos repetidos. Se sim, retorna 0
...
int main(){
    ...
    //Verificação a partir das instâncias
...
int verificar(int **M, int inst){
    ...
    for(int blocoi = 0; blocoi < 3; blocoi++){
        for(int blocoj = 0; blocoj < 3; blocoj++){
            ...
            //Vetor para guardar os elemtos
            int vetor[9] = {0};
            //Conta as passagens para a posição da vetor
            int contagem = 0;
